...
await dbService.query('SELECT * FROM feature_flags WHERE key = $1', [flagKey]);
```
...
await dbService.query(`SELECT * FROM feature_flags WHERE key = '${flagKey}'`);
```
...
![System Architecture Diagram - Placeholder](./diagrams/architecture.png)

### Evaluation Request Sequence

![Evaluation Request Flow - Placeholder](./diagrams/flag-evaluation-flow.png)
