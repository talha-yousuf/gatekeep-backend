...
CREATE TABLE
    feature_flags (
        ...
        rollout_percentage INT NOT NULL CHECK (rollout_percentage BETWEEN 0 AND 100),
...
-- Explicitly targeted users per flag
CREATE TABLE
    targeted_users (
        id SERIAL PRIMARY KEY,
...
-- Audit log for flag mutations history
CREATE TABLE
    audit_log (
        id SERIAL PRIMARY KEY,
        flag_id INT NOT NULL REFERENCES feature_flags (id) ON DELETE CASCADE,
...
-- Admin users (for dashboard / API auth)
CREATE TABLE
    admin_user (
        id SERIAL PRIMARY KEY,
